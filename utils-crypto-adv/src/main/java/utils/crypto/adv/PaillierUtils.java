...
public class PaillierUtils {
    ...
    public static AsymmetricCipherKeyPair generateKeyPair(byte[] seed){
        PaillierKeyPairGenerator generator = new PaillierKeyPairGenerator();
        return generator.generateKeyPair(new SHA256SecureRandom(seed));
    }
    ...
    public static byte[] retrievePublicKey(byte[] privateKey) {
        PaillierPrivateKeyParameters privKey = bytes2PrivKey(privateKey);
        BigInteger multiply = privKey.getQ().multiply(privKey.getP());
        return bigIntegerToBytes(multiply, MODULUS_LENGTH);
    }
