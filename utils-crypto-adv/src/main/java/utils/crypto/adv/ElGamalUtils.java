...
public class ElGamalUtils {
    ...
    public static byte[] pubKey2Bytes_RawKey(ElGamalPublicKeyParameters pubKey) {
        byte[] tmp = pubKey.getY().toByteArray();
    ...
    public static byte[] privKey2Bytes_RawKey(ElGamalPrivateKeyParameters privKey) {
        byte[] tmp = privKey.getX().toByteArray();
        byte[] result = new byte[64];
        if (tmp.length > result.length)
            System.arraycopy(tmp, tmp.length - result.length, result, 0, result.length);
        else System.arraycopy(tmp, 0, result, result.length - tmp.length, tmp.length);
